"""WavLM Configuration for MLX.

This configuration matches the Microsoft wavlm-large checkpoint.
...
class WavLMConfig:
    ...
    Key differences from Wav2Vec2-XLSR:
        - Bucket-based relative position bias (num_buckets=320)
        - GRU-style gating for position embeddings
        - No bias in feature extractor convolutions
    ...
    Attributes:
        ...
        max_bucket_distance: Maximum bucket distance (800)
        do_stable_layer_norm: Use pre-norm (True for WavLM)
    """
    ...
    conv_bias: bool = False  # WavLM has no conv bias
    ...
    # Bucket-based relative position
    num_buckets: int = 320
    ...
    @classmethod
    def wavlm_large(cls) -> "WavLMConfig":
        """Return configuration for wavlm-large checkpoint."""
        return cls()

    @classmethod
    def from_dict(cls, config_dict: dict) -> "WavLMConfig":
        ...
        return cls(
            ...
            max_bucket_distance=config_dict.get("max_bucket_distance", 800),
        )
