...
"""
Phase 9 Benchmark: CTC Path Performance Evaluation.

Comprehensive benchmark of the Rich Audio Understanding CTC path:
- WER (Word Error Rate) on LibriSpeech dev-clean
- CTC forward pass latency
- Real-Time Factor (RTF)
- Per-output type latency breakdown

Usage:
    # Quick benchmark (100 samples)
    python -m tools.whisper_mlx.benchmark_ctc_path --num-samples 100

    # Full benchmark (all dev-clean samples)
    python -m tools.whisper_mlx.benchmark_ctc_path --dataset dev-clean

    # Output to JSON
    python -m tools.whisper_mlx.benchmark_ctc_path --output reports/benchmark_ctc.json

Outputs:
    - WER: Word Error Rate (lower is better)
    - CTC Latency: Time from audio to CTC outputs (ms)
    - RTF: Real-Time Factor (< 1.0 = faster than real-time)
    - Per-frame timing: 50Hz frame rate performance
"""
...
# =============================================================================
# WER Computation (from streaming_eval.py)
# =============================================================================
...
class AggregatedResults:
    ...
    mean_ctc_head_latency_ms: float
...
def run_benchmark(
    ...
    for i, sample in enumerate(samples):
        try:
            ...
            outputs = rich_head(encoder_output)
            mx.eval(outputs)
            ctc_head_latency_ms = (time.time() - t0) * 1000

            # Decode
            t0 = time.time()
            tokens, _ = rich_head.decode_text_greedy(outputs)
            decode_latency_ms = (time.time() - t0) * 1000
            ...
            total_latency_ms = mel_latency_ms + encoder_latency_ms + ctc_head_latency_ms + decode_latency_ms
            ...
            result = BenchmarkResult(
                ...
                ctc_head_latency_ms=ctc_head_latency_ms,
                decode_latency_ms=decode_latency_ms,
                total_latency_ms=total_latency_ms,
                num_frames=encoder_output.shape[1],
            ...
            if verbose:
                ...
                print(f"  Duration: {audio_duration_s:.2f}s, Frames: {result.num_frames}")
    ...
    aggregated = AggregatedResults(
        ...
        mean_ctc_head_latency_ms=np.mean([r.ctc_head_latency_ms for r in results]),
        mean_decode_latency_ms=np.mean([r.decode_latency_ms for r in results]),
...
def main():
    parser = argparse.ArgumentParser(
        description="Phase 9 Benchmark: CTC Path Performance",
    ...
    print("=" * 70)
    print("Phase 9 Benchmark: CTC Path Performance")
    print("=" * 70)
    print()
    ...
    print(f"  CTC Head: {aggregated.mean_ctc_head_latency_ms:.1f}ms")
    print(f"  Decode: {aggregated.mean_decode_latency_ms:.1f}ms")
