...
"""
Train Prosody F0 Conditioning (Phase C) - Version 5

Uses DATA-DRIVEN F0 multipliers from the multilingual dataset (95K samples),
not the RAVDESS-based multipliers that were incorrect for SAD (-4% vs +12%).

Key changes from v4:
1. F0 multipliers derived from actual multilingual dataset analysis
2. Uses 100 diverse training sentences from real emotional speech data
3. Better generalization across languages (EN, TH, JA, RU, FR, EL)

Multiplier comparison (v4 vs v5):
    Emotion     v4 (RAVDESS)   v5 (Multilingual)
    NEUTRAL     1.00           1.00
    ANGRY       1.22           1.07   # v4 was too high
    SAD         1.12           0.96   # v4 was WRONG direction!
    EXCITED     1.21           1.15
    CALM        1.03           1.00   # No F0 difference from neutral
    FRUSTRATED  1.08           1.03
    NERVOUS     1.27           1.09   # v4 was too high
    SURPRISED   1.21           1.26

Usage:
    # Train with multilingual-derived settings
    python scripts/train_prosody_f0_conditioning_v5.py --epochs 50

    # Use orthogonal embeddings (recommended for better emotion differentiation)
    python scripts/train_prosody_f0_conditioning_v5.py \
        --prosody-embeddings models/prosody_embeddings_orthogonal/final.safetensors \
        --epochs 50
"""
...
# DATA-DRIVEN F0 multipliers from multilingual dataset analysis (95K samples)
# Computed from actual F0 measurements, NOT theoretical values
# Key insight: SAD has LOWER F0 than neutral (0.96), not higher!
EMOTION_F0_MULTIPLIERS_V5 = {
    0: 1.00,    # NEUTRAL - baseline
    40: 1.07,   # ANGRY - 7% higher (was 22% in v4)
    41: 0.96,   # SAD - 4% LOWER (was +12% in v4, WRONG!)
    42: 1.15,   # EXCITED - 15% higher (was 21%)
    45: 1.00,   # CALM - same as neutral (was +3%)
    48: 1.03,   # FRUSTRATED - 3% higher (was 8%)
    49: 1.09,   # NERVOUS - 9% higher (was 27%)
    50: 1.26,   # SURPRISED - 26% higher (was 21%)
}

# Default training sentences extracted from multilingual dataset
DEFAULT_SENTENCES_FILE = "data/prosody/training_sentences_v5.txt"
...
def compute_f0_loss(
    ...
    Args:
        ...
        target_multiplier: Target F0 multiplier (e.g., 0.96 for SAD)
...
def train_epoch(
    ...
    # Shuffle sentence-emotion pairs (including NEUTRAL for calibration)
    pairs = [(s, e) for s in sentences for e in emotion_types]
    ...
    for sentence, emotion_type in pairs:
        try:
            ...
            # Get target multiplier from v5 data-driven values
            target_mult = EMOTION_F0_MULTIPLIERS_V5.get(emotion_type, 1.0)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Prosody F0 Conditioning (Phase C) - Version 5 (Multilingual)"
    )
    parser.add_argument(
        "--prosody-embeddings",
        type=str,
        default="models/prosody_embeddings_orthogonal/final.safetensors",
        help="Path to prosody embeddings (default: orthogonal embeddings)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="models/prosody_f0_conditioning_v5",
    ...
    parser.add_argument(
        "--sentences-file",
        type=str,
        default=DEFAULT_SENTENCES_FILE,
        help="Path to file with training sentences",
    )
    ...
    if sentences_path.exists():
        ...
        logger.info(f"Loaded {len(sentences)} sentences from {sentences_path}")
    else:
        logger.error(f"Sentences file not found: {sentences_path}")
        logger.error("Run: python scripts/train_prosody_f0_conditioning_v5.py --help")
        sys.exit(1)

    # Emotion types to train on (all emotions including NEUTRAL)
    emotion_types = list(EMOTION_F0_MULTIPLIERS_V5.keys())
    logger.info(f"Training on {len(emotion_types)} emotion types")

    # Show data-driven multipliers
    logger.info("\nData-driven F0 multipliers (from multilingual dataset):")
    for et, mult in sorted(EMOTION_F0_MULTIPLIERS_V5.items()):
        name = {0: 'NEUTRAL', 40: 'ANGRY', 41: 'SAD', 42: 'EXCITED',
                45: 'CALM', 48: 'FRUSTRATED', 49: 'NERVOUS', 50: 'SURPRISED'}.get(et, f'TYPE_{et}')
        logger.info(f"  {name}: {mult:.2f} ({(mult-1)*100:+.0f}%)")
    ...
    logger.info(f"Training pairs per epoch: {len(sentences) * len(emotion_types)}")
    ...
    config = {
        "version": "v5",
        "description": "Data-driven F0 multipliers from multilingual dataset",
        "multipliers": EMOTION_F0_MULTIPLIERS_V5,
        "sentences_count": len(sentences),
        "epochs": args.epochs,
        "learning_rate": args.lr,
        "best_loss": best_loss,
        "best_mult_error": best_mult_error,
        "prosody_embeddings": args.prosody_embeddings,
    }
