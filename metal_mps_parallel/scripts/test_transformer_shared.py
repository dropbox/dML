#!/usr/bin/env python3
"""Test TransformerEncoderLayer with SHARED model across threads.
This tests if the crash is due to multiple model instances."""
...
def test_shared_model(num_rounds=10):
        ...
        def worker(tid):
            try:
                for i in range(iterations):
                    # Each thread creates its own input
                    x = torch.randn(4, 32, 256, device=device)
                    with torch.no_grad():
                        y = model(x)  # Shared model
...
if __name__ == "__main__":
    ...
    print("Testing with SHARED model (vs separate model per thread)")
    print()
