/**
 * AGX Driver Race Condition Fix - Version 2.7
 *
 * FIXES BUG #047 - Command buffer commit with active encoders (SIGABRT in validate):
 *   v2.5 tracked encoders but didn't prevent command buffers from being
 *   committed while encoders were still active. This caused SIGABRT from
 *   Metal's validation in -[IOGPUMetalCommandBuffer validate].
 *
 *   When synchronizeAllStreams() calls commitAndWait() on a command buffer
 *   that still has an active encoder from another thread, Metal's validation
 *   detects the invalid state and aborts.
 *
 * v2.7 CHANGES from v2.5:
 *   1. Track which command buffer each encoder belongs to
 *   2. Swizzle commit to ensure all encoders are ended first
 *   3. If an encoder is still active at commit time, force-end it gracefully
 *
...
static void encoder_created_v27(id encoder, id commandBuffer) {
    ...
    if (it != g_encoder_states.end()) {
        AGX_LOG("AGX Fix v2.7: Encoder %p already tracked", encoder_ptr);
        return;
    }
    ...
    try {
        ...
        AGX_LOG_ERROR("AGX Fix v2.7: OOM creating state for encoder %p", encoder_ptr);
        return;
    }
    ...
    AGX_LOG("AGX Fix v2.7: Created encoder %p for command buffer %p", encoder_ptr, cb_ptr);
}
...
static void ensure_all_encoders_ended_for_command_buffer(id commandBuffer) {
    ...
    for (void* encoder_ptr : encoders) {
        ...
        if (endImp) {
            AGX_LOG_ERROR("AGX Fix v2.7: Force-ending encoder %p before commit", encoder_ptr);
...
static void agx_fix_v2_7_init() {
    g_log = os_log_create("com.agxfix.v2.7", "main");

    if (getenv(AGX_FIX_DISABLE_ENV)) {
        g_enabled = false;
        os_log(g_log, "AGX Fix v2.7: Disabled via environment");
        return;
    }
    ...
    os_log(g_log, "AGX Fix v2.7: Initializing (with commit-safety for validation fix)");

    id<MTLDevice> device = MTLCreateSystemDefaultDevice();
    if (!device) {
        AGX_LOG_ERROR("AGX Fix v2.7: No Metal device");
        return;
    }
    ...
    if (!encoder || !commandBuffer) {
        AGX_LOG_ERROR("AGX Fix v2.7: Failed to create test objects");
        return;
    }

    g_agx_encoder_class = [encoder class];
    g_agx_command_buffer_class = [commandBuffer class];
    os_log(g_log, "AGX Fix v2.7: AGX Encoder class: %s", class_getName(g_agx_encoder_class));
    os_log(g_log, "AGX Fix v2.7: AGX Command buffer class: %s", class_getName(g_agx_command_buffer_class));
    [encoder endEncoding];
    ...
    @try {
        ...
        if (mpsCommandBuffer) {
            ...
            os_log(g_log, "AGX Fix v2.7: MPS Command buffer class: %s", class_getName(g_mps_command_buffer_class));
        }
    } @catch (NSException* e) {
        os_log(g_log, "AGX Fix v2.7: Exception creating MPSCommandBuffer");
    }
    ...
    if (swizzle_method(g_agx_command_buffer_class, @selector(commit),
        ...
        swizzled_count++;
        os_log(g_log, "AGX Fix v2.7: Swizzled AGX commit (CRITICAL for validation fix)");
    }
    ...
    if (g_mps_command_buffer_class && g_mps_command_buffer_class != g_agx_command_buffer_class) {
        if (swizzle_method(g_mps_command_buffer_class, @selector(commit),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.7: Swizzled MPS commit");
        }
    ...
    os_log(g_log, "AGX Fix v2.7: COMPLETE - %d methods protected (with commit-safety)", swizzled_count);
}
...
extern "C" {
    uint64_t agx_fix_v2_7_get_acquisitions() { return g_mutex_acquisitions.load(); }
    uint64_t agx_fix_v2_7_get_contentions() { return g_mutex_contentions.load(); }
    uint64_t agx_fix_v2_7_get_encoders_created() { return g_encoders_created.load(); }
    uint64_t agx_fix_v2_7_get_encoders_released() { return g_encoders_released.load(); }
    uint64_t agx_fix_v2_7_get_null_impl_skips() { return g_null_impl_skips.load(); }
    uint64_t agx_fix_v2_7_get_method_calls() { return g_method_calls.load(); }
    uint64_t agx_fix_v2_7_get_deferred_releases() { return g_deferred_releases.load(); }
    uint64_t agx_fix_v2_7_get_agx_encoder_creates() { return g_agx_encoder_creates.load(); }
    uint64_t agx_fix_v2_7_get_mps_encoder_creates() { return g_mps_encoder_creates.load(); }
    uint64_t agx_fix_v2_7_get_commit_forced_ends() { return g_commit_forced_ends.load(); }
    uint64_t agx_fix_v2_7_get_commits_intercepted() { return g_commits_intercepted.load(); }
    size_t agx_fix_v2_7_get_active_count() {
        std::lock_guard<std::recursive_mutex> lock(g_encoder_mutex);
        return g_encoder_states.size();
    }
    bool agx_fix_v2_7_is_enabled() { return g_enabled; }
}
