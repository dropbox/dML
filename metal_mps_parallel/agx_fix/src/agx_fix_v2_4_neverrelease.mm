/**
 * AGX Driver Race Condition Fix - Version 2.4 (Never Release)
 *
 * This variant NEVER releases encoders. This trades memory for correctness.
 * Encoders will leak (~KB each) but will never cause use-after-free crashes.
 *
 * FIXES USE-AFTER-FREE BUG IN v2.3:
 *   v2.3 had a race: Thread A could call endEncoding (releasing encoder)
 *   while Thread B was about to call a method on that encoder. The crash
 *   happened in objc_msgSend BEFORE our swizzled method got control.
 *
 * v2.4 ARCHITECTURE:
 *   1. Per-encoder reference counting with "active call" tracking
 *   2. Encoder is retained on creation AND at start of each method call
 *   3. Encoder is released at end of each method call
 *   4. endEncoding marks encoder as "ended" but encoder stays alive
 *      until all in-flight method calls complete
 *   5. This ensures encoder cannot be freed while any thread is using it
 *
 * KEY INSIGHT: The crash at objc_msgSend happens BEFORE our swizzled code
 * runs. We must ensure the encoder object remains valid long enough for
 * objc_msgSend to complete its method lookup.
 *
...
namespace {
    ...
    void store_original_imp(SEL sel, IMP imp) {
        if (g_swizzle_count < MAX_SWIZZLED) {
            ...
            if (g_log) {
                os_log_error(g_log, "AGX Fix v2.4: MAX_SWIZZLED overflow! count=%d, limit=%d, sel=%s",
...
static void encoder_created(id encoder) {
    ...
    if (it != g_encoder_states.end()) {
        AGX_LOG("AGX Fix v2.4: Encoder %p already tracked", ptr);
        return;
    }
    ...
    try {
        ...
        AGX_LOG_ERROR("AGX Fix v2.4: OOM creating state for encoder %p", ptr);
        return;
    }
    ...
    AGX_LOG("AGX Fix v2.4: Created encoder %p (total: %zu)", ptr, g_encoder_states.size());
}
...
static bool encoder_method_begin(id encoder) {
    ...
    if (it == g_encoder_states.end()) {
        ...
        // Try to track it now
        AGX_LOG("AGX Fix v2.4: Untracked encoder %p in method_begin, tracking now", ptr);
        try {
            g_encoder_states.emplace(ptr, EncoderState{});
            CFRetain((__bridge CFTypeRef)encoder);
            g_encoder_states[ptr].retain_count = 1;
            ...
            AGX_LOG_ERROR("AGX Fix v2.4: OOM tracking encoder %p", ptr);
            return false;
        }
    }
...
static void encoder_method_end(id encoder) {
    ...
    if (it == g_encoder_states.end()) {
        AGX_LOG("AGX Fix v2.4: Untracked encoder %p in method_end", ptr);
        return;
    }
    ...
    // NEVER RELEASE: Disabled to prevent use-after-free race
    // CFRelease((__bridge CFTypeRef)encoder);
    // it->second.retain_count--;
    (void)remaining;  // Suppress unused warning

    // NEVER RELEASE: Keep encoder alive forever
    // if (it->second.ended && remaining == 0) {
    //     AGX_LOG("AGX Fix v2.4: Final release of encoder %p (deferred)", ptr);
    //     CFRelease((__bridge CFTypeRef)encoder);
...
static void encoder_ended(id encoder) {
    ...
    if (it == g_encoder_states.end()) {
        AGX_LOG("AGX Fix v2.4: Untracked encoder %p in encoder_ended", ptr);
        return;
    }
    ...
    // NEVER RELEASE: Disabled to prevent use-after-free race
    // Keep encoder alive forever - let it leak
    AGX_LOG("AGX Fix v2.4 NR: Encoder %p ended (kept alive, %d active)",
            ptr, it->second.active_calls);
}

// Force release - called from destroyImpl or dealloc
// NEVER RELEASE variant: This does nothing
static void encoder_force_release(id encoder) {
    ...
    // NEVER RELEASE: Don't release, just log
    AGX_LOG("AGX Fix v2.4 NR: Force release skipped for %p (retain_count=%d)",
            ptr, it->second.retain_count);
    // Don't erase from tracking - encoder stays alive forever
}
...
static bool is_impl_valid(id encoder) {
    ...
    if (impl == nullptr) {
        g_null_impl_skips++;
        AGX_LOG("AGX Fix v2.4: NULL _impl in %p", encoder);
        return false;
    }
    return true;
}
...
static void swizzled_destroyImpl(id self, SEL _cmd) {
    {
        AGXMutexGuard guard;
        AGX_LOG("AGX Fix v2.4: destroyImpl on %p", self);
        encoder_force_release(self);
    }
...
static void agx_fix_v2_4_init() {
    g_log = os_log_create("com.agxfix.v2.4", "main");

    if (getenv(AGX_FIX_DISABLE_ENV)) {
        g_enabled = false;
        os_log(g_log, "AGX Fix v2.4: Disabled via environment");
        return;
    }
    ...
    os_log(g_log, "AGX Fix v2.4: Initializing (active call tracking + deferred release)");

    // Get Metal device and create test objects
    id<MTLDevice> device = MTLCreateSystemDefaultDevice();
    if (!device) {
        AGX_LOG_ERROR("AGX Fix v2.4: No Metal device");
        return;
    }
    ...
    if (!encoder || !commandBuffer) {
        AGX_LOG_ERROR("AGX Fix v2.4: Failed to create test objects");
        return;
    }
    ...
    os_log(g_log, "AGX Fix v2.4: Encoder class: %s", class_getName(g_agx_encoder_class));
    os_log(g_log, "AGX Fix v2.4: Command buffer class: %s", class_getName(g_agx_command_buffer_class));
    ...
    if (blitEncoder) {
        ...
        os_log(g_log, "AGX Fix v2.4: Blit encoder class: %s", class_getName(g_agx_blit_encoder_class));
    ...
    if (dummyTexture) {
        ...
        if (renderEncoder) {
            ...
            os_log(g_log, "AGX Fix v2.4: Render encoder class: %s", class_getName(g_agx_render_encoder_class));
    ...
    if ([commandBuffer4 respondsToSelector:resourceStateSelector]) {
        ...
        if (resourceStateEncoder) {
            ...
            os_log(g_log, "AGX Fix v2.4: Resource state encoder class: %s", class_getName(g_agx_resource_state_encoder_class));
    ...
    if ([commandBuffer5 respondsToSelector:accelStructSelector]) {
        ...
        if (accelStructEncoder) {
            ...
            os_log(g_log, "AGX Fix v2.4: Accel struct encoder class: %s", class_getName(g_agx_accel_struct_encoder_class));
    ...
    if (implIvar) {
        ...
        os_log(g_log, "AGX Fix v2.4: _impl at offset %td", g_impl_ivar_offset);
    } else {
        ...
        while (parent) {
            ...
            if (implIvar) {
                g_impl_ivar_offset = ivar_getOffset(implIvar);
                os_log(g_log, "AGX Fix v2.4: _impl at offset %td in parent %s",
    ...
    // ========================================================================
    // PART 1: Swizzle COMMAND BUFFER encoder creation methods
    // ========================================================================
    ...
    if (swizzle_method(g_agx_command_buffer_class, @selector(computeCommandEncoder),
        ...
        swizzled_count++;
        os_log(g_log, "AGX Fix v2.4: Swizzled computeCommandEncoder");
    }
    ...
    if (swizzle_method(g_agx_command_buffer_class, @selector(computeCommandEncoderWithDescriptor:),
        ...
        swizzled_count++;
        os_log(g_log, "AGX Fix v2.4: Swizzled computeCommandEncoderWithDescriptor:");
    }
    ...
    if (swizzle_method(g_agx_command_buffer_class, @selector(computeCommandEncoderWithDispatchType:),
        ...
        swizzled_count++;
        os_log(g_log, "AGX Fix v2.4: Swizzled computeCommandEncoderWithDispatchType:");
    }
    ...
    if (swizzle_method(g_agx_command_buffer_class, @selector(blitCommandEncoder),
        ...
        swizzled_count++;
        os_log(g_log, "AGX Fix v2.4: Swizzled blitCommandEncoder");
    }
    ...
    if (g_agx_blit_encoder_class) {
        ...
        if (swizzle_method(g_agx_blit_encoder_class, @selector(endEncoding),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled blit endEncoding");
        }
        ...
        if (swizzle_method(g_agx_blit_encoder_class, @selector(deferredEndEncoding),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled blit deferredEndEncoding");
        }

        if (swizzle_method(g_agx_blit_encoder_class, @selector(dealloc),
                           (IMP)swizzled_blit_dealloc, &g_original_blit_dealloc)) {
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled blit dealloc (cleanup fallback)");
        }

        os_log(g_log, "AGX Fix v2.4: Blit encoder methods swizzled");
    }
    ...
    if (g_agx_render_encoder_class) {
        ...
        if (swizzle_method(g_agx_command_buffer_class, @selector(renderCommandEncoderWithDescriptor:),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled renderCommandEncoderWithDescriptor:");
        }
        ...
        if (swizzle_method(g_agx_render_encoder_class, @selector(endEncoding),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled render endEncoding");
        }
        ...
        if (swizzle_method(g_agx_render_encoder_class, @selector(deferredEndEncoding),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled render deferredEndEncoding");
        }

        if (swizzle_method(g_agx_render_encoder_class, @selector(dealloc),
                           (IMP)swizzled_render_dealloc, &g_original_render_dealloc)) {
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled render dealloc (cleanup fallback)");
        }

        os_log(g_log, "AGX Fix v2.4: Render encoder methods swizzled");
    }
    ...
    if (g_agx_resource_state_encoder_class) {
        ...
        if (swizzle_method(g_agx_command_buffer_class, @selector(resourceStateCommandEncoder),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled resourceStateCommandEncoder");
        }
        ...
        if (swizzle_method(g_agx_resource_state_encoder_class, @selector(endEncoding),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled resource state endEncoding");
        }
        ...
        if (swizzle_method(g_agx_resource_state_encoder_class, @selector(dealloc),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled resource state dealloc (cleanup fallback)");
        }

        os_log(g_log, "AGX Fix v2.4: Resource state encoder methods swizzled");
    }
    ...
    if (g_agx_accel_struct_encoder_class) {
        ...
        if (swizzle_method(g_agx_command_buffer_class, @selector(accelerationStructureCommandEncoder),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled accelerationStructureCommandEncoder");
        }
        ...
        if (swizzle_method(g_agx_accel_struct_encoder_class, @selector(endEncoding),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled accel struct endEncoding");
        }
        ...
        if (swizzle_method(g_agx_accel_struct_encoder_class, @selector(dealloc),
            ...
            swizzled_count++;
            os_log(g_log, "AGX Fix v2.4: Swizzled accel struct dealloc (cleanup fallback)");
        }

        os_log(g_log, "AGX Fix v2.4: Acceleration structure encoder methods swizzled");
    }

    os_log(g_log, "AGX Fix v2.4: COMPLETE - %d methods protected (active call tracking)",
           swizzled_count);
}
...
extern "C" {
    uint64_t agx_fix_v2_4_get_acquisitions() { return g_mutex_acquisitions.load(); }
    uint64_t agx_fix_v2_4_get_contentions() { return g_mutex_contentions.load(); }
    uint64_t agx_fix_v2_4_get_encoders_created() { return g_encoders_created.load(); }
    uint64_t agx_fix_v2_4_get_encoders_released() { return g_encoders_released.load(); }
    uint64_t agx_fix_v2_4_get_null_impl_skips() { return g_null_impl_skips.load(); }
    uint64_t agx_fix_v2_4_get_method_calls() { return g_method_calls.load(); }
    uint64_t agx_fix_v2_4_get_deferred_releases() { return g_deferred_releases.load(); }
    size_t agx_fix_v2_4_get_active_count() {
        std::lock_guard<std::recursive_mutex> lock(g_encoder_mutex);
        return g_encoder_states.size();
    }
    bool agx_fix_v2_4_is_enabled() { return g_enabled; }
}
