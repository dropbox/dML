/**
 * AGX Driver Race Condition Fix - Version 2.4
 *
...
static void encoder_ended(id encoder) {
    ...
    if (it->second.active_calls == 0) {
        AGX_LOG("AGX Fix v2.4: Immediate release of encoder %p", ptr);
        CFRelease((__bridge CFTypeRef)encoder);
        it->second.retain_count--;
        g_encoder_states.erase(it);
        g_encoders_released++;
    } else {
        AGX_LOG("AGX Fix v2.4: Deferring release of encoder %p (%d active calls)",
...
static void encoder_force_release(id encoder) {
    ...
    AGX_LOG("AGX Fix v2.4: Force released encoder %p (%d retains)", ptr, count);
    g_encoder_states.erase(it);
    g_encoders_released++;
}
